    ...
    def convert_coordinate(driver: UiDriver, x: int, y: int, source_width: int, source_height: int) -> tuple:
        ...
        try:
            ...
            if x < 0 or y < 0 or source_width <= 0 or source_height <= 0:
                raise ValueError(f"Invalid input parameters: x={x}, y={y}, source_width={source_width}, source_height={source_height}")
            
            # 获取当前设备的屏幕尺寸
            current_width, current_height = CoordinateAdapter.get_device_screen_size(driver, source_width, source_height)
            ...
            if new_x < 0 or new_x > current_width or new_y < 0 or new_y > current_height:
                raise ValueError(f"Converted coordinates ({new_x}, {new_y}) out of screen bounds ({current_width}x{current_height})")
